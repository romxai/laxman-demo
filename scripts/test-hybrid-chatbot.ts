  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      if (result.debug_info) {
        ...
        console.log('  - Confidence:', result.debug_info.nlu_result?.slots?.confidence);
        console.log('  - Products Found:', result.debug_info.retrieval_result?.products?.length || 0);
        console.log('  - Search Criteria:', JSON.stringify(result.debug_info.search_criteria, null, 2));
      }
...
async function testComponents() {
  ...
  try {
    ...
    console.log('❌ NLU Error:', error instanceof Error ? error.message : 'Unknown error');
  }
  ...
  try {
    ...
    if (retrievalResponse.ok) {
      ...
      console.log('✅ Retrieval Result:', JSON.stringify(retrievalResult, null, 2));
    } else {
      console.log('❌ Retrieval Test Failed');
    }
  } catch (error) {
    console.log('❌ Retrieval Error:', error instanceof Error ? error.message : 'Unknown error');
  }
}
